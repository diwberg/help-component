import { FeedbackType, feedbackTypes } from '..'
import { CloseButton } from '../../CloseButton';

interface FeedbackTypeStepProps {
    onFeedbackChanged: (type: FeedbackType) => void;
}

export function FeedbackTypeStep({ onFeedbackChanged }: FeedbackTypeStepProps) {

    return (
        <>
            <header>
                <span className="text-xl leading-6 ">Deixe seu feedback</span>
                <CloseButton />
            </header>
            <div className="flex py-8 gap-2 w-full">
                {Object.entries(feedbackTypes).map(([key, value]) => {
                    return (
                        <div key={key}>
                            <button
                                className="bg-zinc-800 rounded-lg pt-5 w-24 flex-1 flex flex-col items-center gap-2 border-2 border-transparent hover:border-violetDarkA-violetA8 focus:border-violetDarkA-violetA8 focus:outline-none "
                                onClick={() => onFeedbackChanged(key as FeedbackType)}
                            >
                                <img src={value.image.src} alt={value.image.alt} />
                                <span>{value.title}</span>
                            </button>
                        </div>
                    )
                })}
            </div>

        </>
    )
}